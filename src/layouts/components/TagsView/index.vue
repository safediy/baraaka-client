...
<template>
  <div class="tags-view-container">
    <ScrollPane class="tags-view-wrapper" :tag-refs="tagRefs">
      <router-link
        ref="tagRefs"
        v-for="tag in tagsViewStore.visitedViews as any"
        :key="tag.path"
    ...
    <ul v-show="visible" class="contextmenu" :style="{ left: left + 'px', top: top + 'px' }">
      ...
      <li v-if="!isAffix(selectedTag)" @click="closeSelectedTag(selectedTag)">{{ $t('close') }}</li>
      <li @click="closeOthersTags">{{ $t('closeOther') }}</li>
      <li @click="closeAllTags(selectedTag)">{{ $t('closeAll') }}</li>
    </ul>
  </div>
</template>
