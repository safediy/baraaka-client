...
const emits = defineEmits(['personPvzList', 'refresh'])
...
const title = ref<string>(t('add.address'))
...
const showEdit = (editedPvz: LocationInfoDTO) => {
  mapDialogVisible.value = true
  title.value = t('edit.address')
  isEdit.value = true
  Object.assign(editedPvzData, editedPvz)
}

const querySearch = (query: string, cb: (arg: any[]) => void) => {
  if (!autocompleteService.value) {
    autocompleteService.value = new (google as any).maps.places.AutocompleteService()
  }
...
const handleEditPvz = async () => {
  loading.value = true

  if (!editedPvzData.id) {
    ElMessage.error(t('pvz.notFound'))
    return
  }

  try {
    await personPvzService.updatePersonPvz(editedPvzData)
...
watch(centerValue as any, (newValue: any) => {
  yPosition.value = newValue ? 40 : 35
})

defineExpose({ show, close, showEdit })
</script>
...
<template>
  <el-dialog
    ...
    :title="t('add.address')"
    style="max-width: 600px; width: 100%; padding: 20px; border-radius: 24px"
  >
    <div class="h-fit">
      <div class="max-w-[600px] h-full relative my-0 mx-auto" v-loading.fullscreen.lock="loading">
        <div class="head-tab">
          <el-autocomplete
            v-model="state1"
            type="text"
        ...
        >
          ...
          <p class="warn__error-text text-[#6B7280]">{{ t('delivery.notAvailable') }}</p>
        </div>
        ...
        <div class="zoom-controls absolute right-3 bottom-[16%] grid gap-2">
          <button
        ...
        <div class="footer-button absolute right-0 top-[80%] w-full hidden justify-end px-3">
          <button @click="warnHandle">
            <img :src="warnSvg" :alt="warnSvg" />
          </button>
        </div>

        <div class="bottom-[-2%] w-full">
          <div class="flex mt-6 gap-2">
            <el-button
              class="w-full! !h-12 !rounded-[32px] !text-[15px] !font-medium !text-[#111827]"
              @click="closeMapDialog"
            >
              {{ t('cancel') }}
            </el-button>
            <el-button
              class="!w-full !h-12 !rounded-[32px] !text-[15px] !font-medium !text-white !bg-[#030712]"
      ...
      <DynamicDialog v-model="confirmDialog" size="auto" :position-style="positionStyle" :title="''">
        <div class="p-0 flex flex-col bg-white overflow-hidden">
          ...
          <div class="pt-0 px-0 pb-5 text-center">
            <p class="text-[24px] font-semibold text-[#111827] mb-2">
              {{ isEdit ? t('confirm.address.change') : t('confirm.your.location') }}
            </p>
          ...
          <div class="flex items-center justify-center gap-2">
            ...
            <el-button
              @click="isEdit ? handleEditPvz() : handleSubmit()"
              :disabled="loading"
              :loading="loading"
              class="!bg-[#111827] !text-white !font-medium !w-full !h-12"
            >
              <span>{{ isEdit ? t('save') : t('save.address') }}</span>
            </el-button>
          </div>
        </div>
      </DynamicDialog>
...
.warn__error {
  color: #d81b51;
  font-family: 'Transforma Sans_Trial';
  font-size: 14px;
  font-style: normal;
  font-weight: 500;
  line-height: 1.4;
...
.warn__error {
  line-height: 20px;
  @include f-style(13px, 400, #6b7280);
}
