...
import { TelegramSettings, type LayoutSettings } from '@/config/layouts'
...
export const setIsTelegramBot = (settings: TelegramSettings) => {
  localStorage.setItem(CacheKey.TELEGRAM_KEY, JSON.stringify(settings))
}
export const getIsTelegramBot = () => {
  const json = localStorage.getItem(CacheKey.TELEGRAM_KEY)
  return json ? (JSON.parse(json) as TelegramSettings) : null
}

export const removeIsTelegramBot = () => {
  localStorage.removeItem(CacheKey.TELEGRAM_KEY)
}
...
export const getActiveLang = (): LocaleLanguage => {
  const lang = localStorage.getItem(CacheKey.ACTIVE_LANG_NAME)
  if (lang === 'uz' || lang === 'ru' || lang === 'en' || lang === 'zh') {
    return lang
  }
  return 'uz'
}
